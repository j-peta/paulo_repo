...
resource "aws_customer_gateway" "main" {
  bgp_asn    = 65000
  ip_address = "10.10.10.10"   #ip_address = "${azurerm_public_ip.gwpip.ip_address}"
  type       = "ipsec.1"
...
resource "aws_vpn_connection_route" "azure" {
  destination_cidr_block = "142.56.74.2"  #destination_cidr_block = "${azurerm_subnet.subnet.address_prefix}"
